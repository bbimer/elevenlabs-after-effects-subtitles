...
### 4) Transcribe & Compile ANY Video or Audio File (Universal STT)
Extract word-level timestamps and compile semantic subtitles directly from **any video or audio file** (`.mp4`, `.mov`, `.mkv`, `.mp3`, `.wav`, `.srt`, `.vtt`):
```bash
# Transcribe video or audio (uses ElevenLabs Scribe, Groq Whisper, or OpenAI Whisper):
node transcribe.js "C:/Videos/my_promo_reel.mp4"

# Or force a specific STT engine:
node transcribe.js "voiceover.mp3" --engine groq

# Or convert existing subtitle files:
node transcribe.js "subtitles.srt" --out my_custom.captions.json
```
*💡 **Drag & Drop:** You can also simply drag and drop any video or audio file onto `transcribe.bat` in Windows Explorer!*
...
├── transcribe.js                           # Universal Video/Audio/SRT Transcriber & Semantic Compiler
├── transcribe.bat                          # 1-Click Drag & Drop Windows batch runner
