...
(function (thisObj) {
  ...
  // ExtendScript String.prototype.trim polyfill
  ...
  function buildGUI(panelObj) {
    ...
    var ver = header.add('statictext', undefined, 'v2.6 — FreeFlow Studio');
    ...
    // SECTION 5: QUICK TIMELINE SELECTION & BATCH STYLING
        ...
        for (var p = 0; p < data.pages.length; p++) {
          ...
          if (CFG.layoutMode === 1) {
            ...
            for (var li = 0; li < lines.length; li++) {
              ...
              if (createdLayers.length > 0) {
                L.moveBefore(createdLayers[createdLayers.length - 1]);
              } else {
                L.moveBefore(masterBase);
              }
          ...
          // MODE 3: CASCADE LADDER [C]
            ...
            for (var li3 = 0; li3 < lines.length; li3++) {
              ...
              LC.enabled = true;
              if (createdLayers.length > 0) {
                LC.moveBefore(createdLayers[createdLayers.length - 1]);
              } else {
                LC.moveBefore(masterBase);
              }
          ...
          // MODE 2: SINGLE WORD FLASH [W]
            ...
            for (var fw = 0; fw < flatWords.length; fw++) {
              ...
              LW.enabled = true;
              if (createdLayers.length > 0) {
                LW.moveBefore(createdLayers[createdLayers.length - 1]);
              } else {
                LW.moveBefore(masterBase);
              }
          ...
          // MODE 4: HIGHLIGHT TRACKER [T] + [HI]
            ...
            LT.enabled = true;
            if (createdLayers.length > 0) {
              LT.moveBefore(createdLayers[createdLayers.length - 1]);
            } else {
              LT.moveBefore(masterBase);
            }
          ...
          // MODE 5: SINGLE-LINE STREAM (1-LINE CENTER PUNCH) [S]
            ...
            for (var li5 = 0; li5 < lines.length; li5++) {
              ...
              L5.enabled = true;
              if (createdLayers.length > 0) {
                L5.moveBefore(createdLayers[createdLayers.length - 1]);
              } else {
                L5.moveBefore(masterBase);
              }
          ...
          // MODE 6: KARAOKE WORD-FILL (ACTIVE WORD GLOW) [K]
            ...
            for (var li6 = 0; li6 < lines.length; li6++) {
              ...
              LKBase.enabled = true;
              if (createdLayers.length > 0) {
                LKBase.moveBefore(createdLayers[createdLayers.length - 1]);
              } else {
                LKBase.moveBefore(masterBase);
              }
              ...
              for (var wi6 = 0; wi6 < lineWords6.length; wi6++) {
                ...
                LKWord.enabled = true;
                if (createdLayers.length > 0) {
                  LKWord.moveBefore(createdLayers[createdLayers.length - 1]);
                }
